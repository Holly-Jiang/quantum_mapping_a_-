...
vector<int>  qlist= vector<int> ();
vector<int> lolist= vector<int> ();
	vector<int > depthes;
...
void read_ini(std::ifstream& inifile) {
	...
	qlist=vector<int> (positions,-1);
	lolist=vector<int> (nqubits,-1);
	split(line.substr(1,line.size()),qlist,',');
  for (size_t i = 0; i < lolist.size(); i++)
        {
			flag=true;
		if(qlist[i]==20){
			qlist[i]=-1;
	...
	if(!flag){
		cout<<"initial mapping fail!"<<endl;
		exit(4);
	}
}
...
int main(int argc, char** argv) {
	...
	char* bName = argv[1];
	of1 << "FY_VF2_Zulehner_Circuit name: " << bName << " (requires " << nqubits << " qubits)" << endl;

	of1 << endl << "Before mapping: " << endl;
	of1 << "  elementary gates: " << ngates << endl;
	of1 << "  depth: " << layers.size() << endl;

	cout << "FY_VF2_Zulehner_Circuit name: " << bName << " (requires " << nqubits << " qubits)" << endl;
	...
	{	
		qubits[k] = qlist[k];
		locations[k] = lolist[k];
	}
	//	printLocations(qubits);
...
//Fix the position of the single qubit gates
	...
	of2<<"FY_VF2_Zulehner: "<<mingates<<" "<<mindepth<<" "<<minswap<<endl;
	of1<<minId<<"FY_VF2_Zulehner element gates:" << mingates<<" depthes:"<<mindepth<<endl;
