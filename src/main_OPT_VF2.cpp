...
int main(int argc, char** argv) {
	...
	char* bName = argv[1];
	of1 << "OPT_VF2_Circuit name: " << bName << " (requires " << nqubits << " qubits)" << endl;

	of1 << endl << "Before mapping: " << endl;
	of1 << "  elementary gates: " << ngates << endl;
	of1 << "  depth: " << layers.size() << endl;

	cout << "OPT_VF2_Circuit name: " << bName << " (requires " << nqubits << " qubits)" << endl;
	...
	for(int i = 0; i < nqubits; i++) {
		...
		std::ifstream inifile("/root/graph/new/VF2_mapping_opt/data/graphDB/ini_map.my");
		read_ini(inifile);
...
//Fix the position of the single qubit gates
	...
	of2<<"OPT_VF2: "<<mingates<<" "<<mindepth<<" "<<minswap<<endl;
	of1<<minId<<"OPT_VF2 element gates:" << mingates<<" depthes:"<<mindepth<<endl;
